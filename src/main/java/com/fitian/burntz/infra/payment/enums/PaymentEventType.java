...
public enum PaymentEventType {
  ...
  TEMPORARY_ENTITLEMENT_GRANT("TEMPORARY_ENTITLEMENT_GRANT", "RevenueCat이 해당 스토어에서 구매를 일시적으로 검증할 수 없어 고객에게 단기 권한을 부여했습니다. 이 이벤트는 예외적인 상황(예: 앱 스토어 부분 중단)에서 전송되며 고객이 구매를 했지만 권한을 얻지 못하는 것을 방지하는 데 사용됩니다. 권한의 만료일은 항상 최대 24시간 후입니다. 예외적인 상황이 해결되고 RevenueCat이 구매를 검증할 수 있게 되면 일반 INITIAL_PURCHASE이벤트가 전송됩니다. 구매를 검증할 수 없는 경우 EXPIRATION동일한 이벤트가 transaction_id전송됩니다. 참고: 이 이벤트 유형은 스토어 서버와의 연결이 제한된 경우에 전송되므로 일반 구매 이벤트보다 정보가 적습니다."),
